...
import 'package:placement_app/repository/api/student/apply_jobs_screen/model/company_details_model.dart';
...
class ApplyJobsController extends ChangeNotifier {
  ...
  postApplyJob(context, id) async {
    ...
    ApplyJobsService.postApplyJob(id).then((value) {
      if (value["status"] == 1) {
        ...
        if (value['msg'] == null) {
          AppUtils.oneTimeSnackBar("Failed To Apply", context: context);
        } else {
  ...
  fetchCompanyDetails(context, id) async {
    log("ApplyJobsController -> fetchCompanyDetails()");
    ApplyJobsService.fetchCompanyDetails(id).then((value) {
      if (value["status"] == 1) {
        companyDetailsModel = CompanyDetailsModel.fromJson(value);
        isLoadingCD = false;
      } else {
        AppUtils.oneTimeSnackBar("error", context: context);
      }
